...
Results_Package rkYolov5s::infer(Frame_Package frame_package)
{
    ...
    for (int i = 0; i < detect_result_group.count; i++)
    {
        ...
        detection_results.score = det_result->prop * 100;
        detection_results.name = det_result->name;
    ...
    for(int i = 0 ; i < Results_package.Results.size() ; i++){
        if(Results_package.Results[i].name == "car" && Results_package.Results[i].score > 40){

                //printf("%s @ (%d %d %d %d) %f\n", Results_package.Results[i].name.c_str(), Results_package.Results[i].x1, Results_package.Results[i].y1,
                //Results_package.Results[i].x2,Results_package.Results[i].y2, Results_package.Results[i].score);
                sprintf(text, "%s %.1f%%", Results_package.Results[i].name.c_str(), Results_package.Results[i].score);
                rectangle(Results_package.frame_origin, cv::Point(Results_package.Results[i].x1, Results_package.Results[i].y1), cv::Point(Results_package.Results[i].x2, Results_package.Results[i].y2), cv::Scalar(0, 120, 0, 128), 2);
                putText(Results_package.frame_origin, text, cv::Point(Results_package.Results[i].x1, Results_package.Results[i].y1 -20), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255));
                
            }   
    }
   // Results_package.id = frame_package.id;
   // Results_package.type = 'A';
