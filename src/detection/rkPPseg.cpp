...
rkPPseg::rkPPseg(const std::string &model_path)
{
    this->model_path = model_path;
}


int rkPPseg::init(rknn_context *ctx_in, bool share_weight)
{
...
rknn_context *rkPPseg::get_pctx()
{
    return &ctx;
}

Results_Package rkPPseg::infer(Frame_Package frame_package)
{
    ...
    cv::resize(frame_package.frame, frame_package.frame, cv::Size(512, 512));
    // 后处理/Post-processing
    float *output = (float *)outputs[0].buf;
    //sigmoid(output , output_attrs[0].n_elems );
    ret = draw_segment_image(output, frame_package.frame);
...
Color rkPPseg::getColorById(int id) {
...
int rkPPseg::draw_segment_image(float* result, cv::Mat& result_img) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            Color foundColor;
            // 根据类别索引获取颜色
            if(maxClassIndex == 13 || maxClassIndex == 0){
